...
export function Journey() {
  ...
  return (
    <section ref={ref} data-motion-skip className="relative bg-[#050B16]" style={reduced ? undefined : { height: `${steps.length * 90}vh` }}>
      <div className={reduced ? "section-y" : "sticky top-[var(--ticker-h)] h-[calc(100svh-var(--ticker-h))] overflow-hidden flex flex-col justify-center"}>
        ...
        <motion.div style={reduced ? undefined : { x }} className={reduced ? "shell grid gap-5 md:grid-cols-2" : "flex gap-8 px-[5vw] will-change-transform"}>
          {steps.map((s) => (
            <article key={s.n} className={`${reduced ? "w-full" : "shrink-0 w-[78vw] sm:w-[60vw] lg:w-[48vw] max-w-[720px]"} rounded-3xl border border-white/[0.08] bg-gradient-to-br from-white/[0.04] to-transparent p-10 lg:p-14`}>
