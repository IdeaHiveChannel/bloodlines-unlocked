  ...
  return (
    ...
    >
      <button
        ...
        aria-haspopup="true"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1 whitespace-nowrap text-[0.8125rem] font-medium tracking-tight text-[var(--ink-dim)] transition-colors hover:text-white lg:text-[0.8125rem] xl:text-[0.875rem] html-ml:lg:text-[0.68rem] html-ml:xl:text-[0.78rem]"
        data-cursor="link"
      >
        {label}
        <ChevronDown
            ...
            <ul className={columns === 2 ? "grid grid-cols-2 gap-x-1" : ""}>
              {links.map((l) => (
                <li key={l.to + l.label}>
                  <LocaleLink
                    to={l.to}
                    activeProps={{ className: "block rounded-xl px-3 py-2 text-small bg-white/[0.06] text-[var(--accent)]" }}
